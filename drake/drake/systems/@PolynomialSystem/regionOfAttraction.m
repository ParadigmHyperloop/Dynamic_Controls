...
if nargin<2 || isa(varargin{1},'Point')
  ...
  %tons of numerical problems. I guess clean() is the magic function.
  f = clean(f);
...
function V = bilinear(V0,f,options)
  ...
  for iter=1:options.max_iterations
    ...
    Vreg = subs(Vbal,x,inv(T)*x);
    Vreg = clean(Vreg);
    V = SpotPolynomialLyapunovFunction(V.getFrame,Vreg);
    plotFunnel(V); 
    plotFunnel(V0/rho,struct('color',[.9 .3 .2])); drawnow;
    
    % check for convergence
    if ((vol - last_vol) < options.converged_tol*last_vol)
    %if (abs(vol - last_vol) < options.converged_tol*last_vol)   
      break;
    end
  end
...
function [V,rho]=optimizeV(x,f,L1,L2,V0,sigma1,Vxmonom,options)
  ...
  %force the origin to have V(0) = 0
  %is missing from the original drake implementation
  %and is blowing up the bilinear solver
  prog = prog.withEqs(subs(V, x, zeros(length(x), 1))); %V(0) = 0
  ...
  %there may (will probably) be numerical problems, and V(0) ~= 0.
  %make it so.
 % V_offset = subs(V, x, zeros(length(x), 1));
 % V = V - V_offset;
  V = clean(V);
